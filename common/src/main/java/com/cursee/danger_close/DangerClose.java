...
// todo finish copying networking from disenchanting table for config sync
public class DangerClose {
    ...
    public static void detect(LivingEntity entity) {
        ...
        if (entity.onGround()) {
            ...
            boolean hasEnchantedFeetSlotItem = feetItemStack != ItemStack.EMPTY && EnchantmentHelper.hasAnyEnchantments(feetItemStack);
            boolean hasFrostWalker = hasEnchantedFeetSlotItem && feetItemStack.getEnchantments().keySet().stream().anyMatch(enchantmentHolder -> enchantmentHolder.is(Enchantments.FROST_WALKER));
